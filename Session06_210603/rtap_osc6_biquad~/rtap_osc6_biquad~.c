...
static t_class *rtap_osc6_biquad_tilde_class;
...
typedef struct rtap_osc6_biquad_tilde
{
    t_object  x_obj;
    t_sample f;
    vas_osc *osc;
...
t_int *rtap_osc6_biquad_tilde_perform(t_int *w)
{
...
void rtap_osc6_biquad_tilde_dsp(rtap_osc6_tilde *x, t_signal **sp)
{
    dsp_add(rtap_osc6_biquad_tilde_perform, 4, x, sp[0]->s_vec, sp[1]->s_vec, sp[0]->s_n);
}
...
void rtap_osc6_biquad_tilde_free(rtap_osc6_tilde *x)
{
    outlet_free(x->out);
    vas_osc_free(x->osc);
...
void rtap_biquad_tilde_setFrequency(rtap_osc6_tilde *x, float freq)
{
   vas_iir_biquad_setFrequency(x->biquad, freq);
}

void *rtap_osc6_biquad_tilde_new(t_floatarg f)
{
    rtap_osc6_tilde *x = (rtap_osc6_tilde *)pd_new(rtap_osc6_biquad_tilde_class);
    
    //The main inlet is created automatically
    x->out = outlet_new(&x->x_obj, &s_signal);
    x->osc = vas_osc_new(44100);
    x->biquad = vas_iir_biquad_new(VAS_IIR_BIQUAD_LOWPASS, 1000, 20);
...
void rtap_osc6_biquad_tilde_setup(void)
{
      rtap_osc6_biquad_tilde_class = class_new(gensym("rtap_osc6_biquad~"),
            (t_newmethod)rtap_osc6_biquad_tilde_new,
            (t_method)rtap_osc6_biquad_tilde_free,
        sizeof(rtap_osc6_tilde),
            CLASS_DEFAULT,
            A_DEFFLOAT, 0);

      class_addmethod(rtap_osc6_biquad_tilde_class, (t_method)rtap_osc6_biquad_tilde_dsp, gensym("dsp"), 0);

      // this adds the gain message to our object
      class_addmethod(rtap_osc6_biquad_tilde_class, (t_method)rtap_osc6_tilde_noteOn, gensym("noteon"), A_DEFFLOAT, A_DEFFLOAT, 0);
      class_addmethod(rtap_osc6_biquad_tilde_class, (t_method)rtap_osc6_tilde_setExternTable, gensym("setwaveform"), A_SYMBOL, 0);
      class_addmethod(rtap_osc6_biquad_tilde_class, (t_method)rtap_osc6_tilde_setEnvelopeTable, gensym("setenvelope"), A_SYMBOL, 0);
      
        
    // this adds the gain message to our object
    class_addmethod(rtap_osc6_biquad_tilde_class, (t_method)rtap_biquad_tilde_setFrequency, gensym("freq"), A_DEFFLOAT,0);
      CLASS_MAINSIGNALIN(rtap_osc6_biquad_tilde_class, rtap_osc6_tilde, f);
}
